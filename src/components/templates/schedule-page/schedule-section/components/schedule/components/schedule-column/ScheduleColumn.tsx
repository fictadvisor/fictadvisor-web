...
import ScheduleCard from '@/components/templates/schedule-page/schedule-section/components/schedule/components/schedule-column/components/schedule-card';
...
const ScheduleColumn: FC<ScheduleColumnProps> = ({ day }) => {
  ...
  const handleClick = async (_event: Event, week: string | number) => {
    if (!user) {
      toast.info(
        'Неавторизован(ий/а) 🐸',
        'Увійди в акаунт для перегляду детальної інформації про подію',
        3000,
      );
      return;
    }
